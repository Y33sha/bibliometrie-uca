...
from interfaces.api.async_deps import subjects_queries
...
async def list_subjects(
    ...
    queries: AsyncSubjectsQueries = Depends(subjects_queries),
) -> Any:
    """Liste paginée des sujets, ordonnée par `usage_count` décroissant."""
    offset = (page - 1) * per_page
    items = await queries.list_subjects(q=q, limit=per_page, offset=offset, min_count=min_count)
    total = await queries.count_subjects(q=q, min_count=min_count)
...
async def get_subject(
    ...
    subject = await queries.get_subject(subject_id)
    if subject is None:
        raise HTTPException(status_code=404, detail="Subject not found")
    neighbors = await queries.get_subject_neighbors(
