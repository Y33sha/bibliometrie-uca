...
def main() -> None:
    ...
    try:
        ...
        pub_metrics = run_resolve_publishers(
            logger,
            repo=doi_prefix_repository(conn),
        ...
        if not args.dry_run:
            conn.commit()
        logger.info("✓ resolve_ra — %s", ra_metrics.as_summary())
        logger.info("✓ resolve_publishers — %s", pub_metrics.as_summary())
    finally:
        conn.close()
