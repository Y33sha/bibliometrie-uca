...
Implémente le port `application.ports.admin_feedback_queries.AsyncAdminFeedbackQueries`
via `PgAsyncAdminFeedbackQueries`. Conformité au Protocol par duck
typing — pas d'import depuis `application/` (règle DDD).
"""
...
class PgAsyncAdminFeedbackQueries:
    """Adapter SA pour `AsyncAdminFeedbackQueries`."""
    ...
    async def feedback_false_negatives(
        ...
        return await self._feedback_paginated(
...
class PgAdminFeedbackQueries:
    """Variante sync de `PgAsyncAdminFeedbackQueries`."""
