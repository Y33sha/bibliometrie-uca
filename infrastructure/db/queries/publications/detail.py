...
async def all_years(conn: AsyncConnection) -> list[int]:
...
async def _fetch_biblio_source_authorships(
    conn: AsyncConnection, source: str, pub_id: int
) -> list[dict[str, Any]]:
...
async def get_publication_detail(conn: AsyncConnection, pub_id: int) -> dict[str, Any] | None:
    ...
    hal_authorships = await _fetch_biblio_source_authorships(conn, "hal", pub_id)
    oa_authorships = await _fetch_biblio_source_authorships(conn, "openalex", pub_id)
    wos_authorships = await _fetch_biblio_source_authorships(conn, "wos", pub_id)
    scanr_authorships = await _fetch_biblio_source_authorships(conn, "scanr", pub_id)
    ...
    subjects = await get_publication_subjects(conn, pub_id)
...
async def get_publication_subjects(conn: AsyncConnection, pub_id: int) -> list[dict[str, Any]]:
