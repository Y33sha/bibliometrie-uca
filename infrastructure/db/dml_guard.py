...
def _on_savepoint(conn: Connection, name: str) -> None:
    # Mémorise l'état du flag à l'ouverture du savepoint : un rollback ultérieur
    # ramènera la connexion à cet état (le DML émis depuis sera annulé).
...
def _on_rollback_savepoint(conn: Connection, name: str, context: object) -> None:
    ...
    if stack.pop():
        ...
        # Aucun DML non committé avant le savepoint : ce qui a été émis depuis est
        # annulé (cas d'un preview écrit puis rollbacké), le flag retombe à zéro.
        conn.info.pop(_DIRTY_KEY, None)


def _on_release_savepoint(conn: Connection, name: str, context: object) -> None:
    # Savepoint relâché (ses écritures intégrées à la transaction) : on garde le flag
    # courant, on dépile seulement.
    stack = conn.info.get(_STACK_KEY)
    if stack:
        stack.pop()
