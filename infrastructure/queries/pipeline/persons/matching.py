...
def fetch_out_of_perimeter_candidates(conn: Connection) -> list[BareUnlinkedAuthorship]:
    """Candidats hors-périmètre (`in_perimeter = FALSE`) rattachables sans forme de nom : par identifiant fort partagé avec une personne connue, ou par ancrage cross-source (même publication × position qu'un authorship déjà rattaché).

    Union des quatre branches d'accès, dédupliquée en SQL (`UNION`) : une même signature peut être candidate par plusieurs chemins. La cascade côté orchestrateur arbitre ensuite (les barreaux nom/création y sont neutralisés pour ces candidats hors-périmètre).
...
def fetch_name_form_map(conn: Connection) -> dict[str, list[int]]:
    ...
    Agrégation par `name_form` sur la table dénormalisée `(name_form, person_id, sources[], status)` : un dict trié par `person_id` croissant pour stabilité.
