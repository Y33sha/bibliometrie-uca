...
async def unlink_authorship(conn: Any, person_id: int, source: str, authorship_id: int) -> None:
    if isinstance(conn, AsyncConnection):
        await conn.execute(
            text("""
...
async def assign_orphan_sa(
    conn: Any, person_id: int, source: str, authorship_id: int
) -> dict | None:
    if isinstance(conn, AsyncConnection):
...
async def batch_assign_orphans(conn: Any, person_id: int, sa_ids: list[int]) -> int:
    if not sa_ids:
        return 0
    ...
    if isinstance(conn, AsyncConnection):
        ...
        for form in forms:
            await _name_forms.add_name_form(conn, person_id, form)
...
async def count_authorships_with_name_form(conn: Any, person_id: int, name_form: str) -> int:
    if isinstance(conn, AsyncConnection):
        result = await conn.execute(
            text(f"""
