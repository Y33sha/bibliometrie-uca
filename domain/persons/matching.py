...
def decide_cross_source_match(
    ...
    """Décide si une authorship peut être rattachée à une `person` déjà associée à la même publication × position auteur via une autre source.

    Le pipeline ingère chaque publi par 1+ sources. Quand par exemple HAL a déjà rattaché l'auteur en position 3 à person 42, et qu'on rencontre l'authorship OpenAlex en position 3 de la même publi non encore rattachée, on peut transmettre le rattachement — à condition que le nom soit compatible (garde-fou contre les désalignements de position entre sources).

    Paramètres :

    - `authorship_source` : source de l'authorship qu'on cherche à rattacher. Sert à exclure les candidats de la même source (qui ne porteraient aucun signal supplémentaire).
    - `last_norm`, `first_norm` : nom normalisé de l'authorship.
    - `candidates` : liste de tuples `(person_id, last_norm, first_norm, source)` — les authorships déjà rattachées à la même `(publication_id, author_position)`, à fournir via prefetch (typiquement `linked_index[(pub_id, position)]`).

    Cascade :

    - Aucun candidat compatible → `None`.
    - Candidats compatibles convergent tous vers la même `person_id` → cette `person_id`.
    - Candidats compatibles divergent (≥ 2 `person_id` distincts) → `None` (conflit, pas de match safe).
    """
...
class IdentifierMatch:
    ...
    - `person_id` : la personne si l'identifiant résout **et** que son nom est compatible avec la signature ; `None` sinon.
    - `rejection` : `(person_id, target_name)` quand l'identifiant résolvait vers une personne mais que son nom est jugé incompatible avec la signature — le match est refusé, et l'info sert à journaliser le rejet (identifiant + les deux formes).
    """
...
def form_matches_person(
    ...
    Vrai si `form` est compatible (`names_compatible`, comparaison par tokens) avec le nom-prénom de la personne, ou avec l'une de ses formes de nom **confirmées** (un nom validé par un humain que le nom-prénom canonique ne recouvre pas, changement de nom inclus).

    Sert à arbitrer un conflit d'attribution d'identifiant : la forme majoritaire des signatures portant une valeur (le *consensus*, agrégat ordre-indépendant fourni par le caller) est confrontée au propriétaire actuel et au candidat en conflit — l'identifiant se transfère vers le candidat si, et seulement si, le consensus le désigne, lui et pas le propriétaire.
    """
...
def decide_person_match(
    *,
    ...
    Pure, testable sans BDD : les paramètres sont précalculés par le caller via prefetch.
    """
