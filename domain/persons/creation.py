...
def allow_person_creation(source: str, roles: list[str]) -> bool:
    ...
    Note : distincte de la règle `OUT_OF_SCOPE_DOC_TYPES` (cf. `domain.publications.scope`) qui exclut entièrement memoir et peer_review du pipeline persons (ni matching ni création). Ici on discute du cas inverse : matching autorisé, création interdite.
    """
