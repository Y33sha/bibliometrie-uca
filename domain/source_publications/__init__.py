...
Une `SourcePublication` est l'image d'un document dans une source externe (HAL, OpenAlex, WoS, theses.fr, ScanR, …), agrégée plus tard dans la `Publication` canonique. Entité de lecture : identité `(source, source_id)` / `id`, immuable et utilisée en lecture seule ; jamais persistée via cet objet (les écritures passent par le SQL).
