...
Quand une correction écrit la valeur **effective** dans une colonne typée de `source_publications`, le brut source écrasé est stashé dans la colonne JSONB `raw_metadata`, par champ, au format :
...
def hydrate_raw_view[ViewT: DataclassInstance](
    ...
    """Reconstruit la vue aux valeurs **source d'origine** (avant correction), en réinjectant chaque champ stashé dans `raw_metadata`. Les champs jamais corrigés gardent leur valeur courante. Inverse de la persistance des corrections.

    Générique : réinjecte tout champ présent dans `raw_metadata` *et* sur la vue, sans liste figée — un champ corrigeable de plus est pris en charge sans modification ici.
    """
    if not raw_metadata:
        return view
    overrides = {
