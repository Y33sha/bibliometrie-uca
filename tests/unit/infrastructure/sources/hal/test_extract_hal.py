...
class TestBuildQuery:
    def test_combines_years_and_since(self, adapter):
        # Mode daily : les notices HAL récemment modifiées *dans* la fenêtre d'années.
        assert adapter.build_query(years=[2024, 2026], since="2026-05-01") == (
            "producedDateY_i:[2024 TO 2026] AND modifiedDate_tdate:[2026-05-01T00:00:00Z TO *]"
        )

    def test_since_alone(self, adapter):
        assert adapter.build_query(years=None, since="2025-12-31") == (
            "modifiedDate_tdate:[2025-12-31T00:00:00Z TO *]"
        )
