...
def _avancement() -> Progression:
    return Progression(None, "OpenAlex", None)
...
def _adapter(pages: list[dict], comptes: list[BatchInsertCounts] | None = None) -> MagicMock:
    ...
    a.count.return_value = sum(len(p["results"]) for p in pages)
...
def test_pagination_jusqu_a_l_epuisement_du_curseur():
    ...
    assert extract_year(adapter, MagicMock(), ["I1"], _LOGGER, _avancement(), year=2024) == (
        3,
        0,
        0,
    )
...
def test_ventilation_cumulee_sur_les_pages():
    ...
    assert extract_year(adapter, MagicMock(), ["I1"], _LOGGER, _avancement()) == (1, 1, 3)
...
def test_page_sans_resultat_interrompt():
    ...
    assert extract_year(adapter, MagicMock(), ["I1"], _LOGGER, _avancement()) == (0, 0, 0)
...
def test_run_parcourt_les_annees_de_la_configuration():
    ...
    assert [appel.kwargs["year"] for appel in adapter.count.call_args_list] == [2023, 2024]
...
def test_run_incremental_fait_une_seule_passe_sur_la_date():
    ...
    assert adapter.count.call_args.kwargs["since"] == "2026-01-01"
