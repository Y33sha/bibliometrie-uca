...
class TestStep0HalAccounts:
    def test_existing_person_propagates(self, sa_sync_conn):
        ...
        pub1 = _insert_publication(sa_sync_conn, "Pub 1")
        ...
        sa_sync_conn.execute(
            ...
            {"pid": person_id, "sp_id": ha},
        )

        hd1 = _insert_hal_document(sa_sync_conn, "hal-001", pub1)
        hd2 = _insert_hal_document(sa_sync_conn, "hal-002", pub2)
        has1 = _insert_hal_authorship(sa_sync_conn, hd1, ha, position=0)
        has2 = _insert_hal_authorship(sa_sync_conn, hd2, ha, position=0)
        ...
        assert _get_person_id_of_hal_authorship(sa_sync_conn, has1) == person_id
        assert _get_person_id_of_hal_authorship(sa_sync_conn, has2) == person_id

    def test_virgin_hal_account_skipped(self, sa_sync_conn):
        ...
        pub = _insert_publication(sa_sync_conn, "Pub vierge")
        ha = _insert_hal_author(sa_sync_conn, "Népomucène Bensoussan", hal_person_id=99999)
        hd = _insert_hal_document(sa_sync_conn, "hal-virgin", pub)
        has_id = _insert_hal_authorship(sa_sync_conn, hd, ha, position=0)
        ...
        assert _get_person_id_of_hal_authorship(sa_sync_conn, has_id) is None
        # L'authorship ne doit pas être marquée comme traitée
        assert ("hal", has_id) not in linked_ids

    def test_virgin_hal_account_matched_by_name(self, sa_sync_conn):
        ...
        ha = _insert_hal_author(
            sa_sync_conn, "Népomucène Bensoussan", hal_person_id=88888, orcid="0000-0001-1111-2222"
        )
        hd = _insert_hal_document(sa_sync_conn, "hal-nepo", pub)
        has_id = _insert_hal_authorship(sa_sync_conn, hd, ha, position=0)
        ...
        assert _get_person_id_of_hal_authorship(sa_sync_conn, has_id) == person_id
