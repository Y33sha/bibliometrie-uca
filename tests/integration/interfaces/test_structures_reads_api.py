"""Tests de caractérisation des lectures de structure servant la page publique des laboratoires.

Les quatre lectures s'exercent sur une base vide : ce qu'on
...
class TestLaboratoryAddresses:
    def test_paginated(self, client):
        r = client.get("/api/structures/999999999/addresses", params={"page": 1, "per_page": 10})
        assert r.status_code == 200
        body = r.json()
        assert body["page"] == 1
    ...
    def test_rejects_per_page_above_ceiling(self, client):
        r = client.get("/api/structures/1/addresses", params={"per_page": 500})
        assert r.status_code == 422


class TestLaboratoryDashboard:
    def test_returns_dashboard(self, client):
        r = client.get("/api/structures/999999999/dashboard")
        assert r.status_code == 200


class TestLaboratorySubjects:
    def test_empty_without_publication(self, client):
        r = client.get("/api/structures/999999999/subjects")
        assert r.status_code == 200
        assert r.json() == []

    def test_honours_limit(self, client):
        r = client.get("/api/structures/999999999/subjects", params={"limit": 5})
        assert r.status_code == 200
