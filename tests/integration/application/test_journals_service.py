...
class TestFindOrCreatePublisher:
    def test_returns_none_on_empty_name(self, sa_sync_conn, pub_repo):
        assert find_or_create_publisher(sa_sync_conn, None, repo=pub_repo) is None
        assert find_or_create_publisher(sa_sync_conn, "", repo=pub_repo) is None

    def test_creates_new_publisher(self, sa_sync_conn, pub_repo):
        pub_id = find_or_create_publisher(sa_sync_conn, "Elsevier", repo=pub_repo)
    ...
    def test_finds_existing_by_openalex_id(self, sa_sync_conn, pub_repo):
        ...
        found = find_or_create_publisher(
            sa_sync_conn, "Elsevier BV", openalex_id="P4310310871", repo=pub_repo
        )
        assert found == existing

    def test_finds_existing_by_name_form(self, sa_sync_conn, pub_repo):
        existing = find_or_create_publisher(sa_sync_conn, "Elsevier", repo=pub_repo)
        found = find_or_create_publisher(sa_sync_conn, "elsevier", repo=pub_repo)
        assert found == existing

    def test_attaches_openalex_id_if_missing(self, sa_sync_conn, pub_repo):
        existing = find_or_create_publisher(sa_sync_conn, "Elsevier", repo=pub_repo)
        find_or_create_publisher(sa_sync_conn, "Elsevier", openalex_id="P123", repo=pub_repo)
        row = _fetch_one(
...
class TestFindOrCreateJournal:
    def test_returns_none_on_empty_title(self, sa_sync_conn, repo):
        assert find_or_create_journal(sa_sync_conn, None, repo=repo) is None
        assert find_or_create_journal(sa_sync_conn, "", repo=repo) is None

    def test_creates_new_journal(self, sa_sync_conn, repo):
        j_id = find_or_create_journal(sa_sync_conn, "Nature", issn="0028-0836", repo=repo)
    ...
    def test_finds_by_openalex_id(self, sa_sync_conn, repo):
        ...
        found = find_or_create_journal(
            sa_sync_conn, "Nature Journal", openalex_id="S137773608", repo=repo
        )
        assert found == existing

    def test_finds_by_issn(self, sa_sync_conn, repo):
        existing = _insert_journal(sa_sync_conn, "Nature", issn="0028-0836")
        found = find_or_create_journal(sa_sync_conn, "Nature Variant", issn="0028-0836", repo=repo)
        assert found == existing

    def test_finds_by_eissn(self, sa_sync_conn, repo):
        existing = _insert_journal(sa_sync_conn, "Nature", eissn="1476-4687")
        found = find_or_create_journal(sa_sync_conn, "Nature", eissn="1476-4687", repo=repo)
        assert found == existing

    def test_finds_by_issnl(self, sa_sync_conn, repo):
        existing = _insert_journal(sa_sync_conn, "Nature", issnl="0028-0836")
        found = find_or_create_journal(sa_sync_conn, "Other Title", issnl="0028-0836", repo=repo)
        assert found == existing

    def test_finds_by_name_form(self, sa_sync_conn, repo):
        find_or_create_journal(sa_sync_conn, "Nature", repo=repo)
        found = find_or_create_journal(sa_sync_conn, "nature", repo=repo)
    ...
    def test_enriches_metadata_on_match(self, sa_sync_conn, repo, pub_repo):
        ...
        existing = _insert_journal(sa_sync_conn, "Nature", issn="0028-0836")
        pub_id = find_or_create_publisher(sa_sync_conn, "Springer", repo=pub_repo)
        find_or_create_journal(
            sa_sync_conn,
            "Nature",
            issn="0028-0836",
...
class TestUpdateJournalApc:
    ...
    def test_coalesce_preserves_existing(self, sa_sync_conn, repo):
        ...
        update_journal_apc(sa_sync_conn, j_id, apc_currency="EUR", repo=repo)
        row = _fetch_one(
            sa_sync_conn,
            "SELECT apc_amount, apc_currency, is_in_doaj FROM journals WHERE id = :id",
            id=j_id,
...
class TestUpdateJournal:
    ...
    def test_partial_update(self, sa_sync_conn, repo):
        ...
        update_journal(sa_sync_conn, j, fields={"eissn": "1476-4687"}, repo=repo)
...
class TestResetJournalApc:
    def test_resets_only_openalex_journals(self, sa_sync_conn, repo):
        ...
        n = reset_journal_apc(sa_sync_conn, repo=repo)
...
class TestMergePublishers:
    def test_raises_on_self_merge(self, sa_sync_conn, repo, pub_repo):
        ...
        with pytest.raises(ConflictError, match="lui-même"):
            merge_publishers(sa_sync_conn, p_id, p_id, publisher_repo=pub_repo, journal_repo=repo)
    ...
    def test_transfers_journals_and_deletes_source(self, sa_sync_conn, repo, pub_repo):
        ...
        merge_publishers(sa_sync_conn, target, source, publisher_repo=pub_repo, journal_repo=repo)
...
class TestMergeJournals:
    def test_raises_on_self_merge(self, sa_sync_conn, repo):
        j_id = _insert_journal(sa_sync_conn, "Nature")
        with pytest.raises(ConflictError, match="lui-même"):
            merge_journals(sa_sync_conn, j_id, j_id, repo=repo)
    ...
    def test_transfers_publications(self, sa_sync_conn, repo):
        ...
        merge_journals(sa_sync_conn, target, source, repo=repo)
