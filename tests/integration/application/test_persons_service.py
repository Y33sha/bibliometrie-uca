...
class TestRemoveIdentifier:
    def test_removes_existing(self, db, repo):
        p = _insert_person(db)
        add_identifier(db, p, "orcid", "0000-0001", repo=repo)
        remove_identifier(db, p, "orcid", "0000-0001", repo=repo)
    ...
    def test_raises_not_found(self, db, repo):
        ...
        with pytest.raises(NotFoundError):
            remove_identifier(db, p, "orcid", "unknown", repo=repo)


class TestUpdateIdentifierStatus:
    def test_sets_status(self, db, repo):
        p = _insert_person(db)
        add_identifier(db, p, "orcid", "0000-0001", repo=repo)
        db.execute("SELECT id FROM person_identifiers WHERE id_value='0000-0001'")
        ident_id = db.fetchone()["id"]

        row = update_identifier_status(db, ident_id, "confirmed", repo=repo)

        assert row["status"] == "confirmed"

    def test_raises_not_found(self, db, repo):
        with pytest.raises(NotFoundError):
            update_identifier_status(db, 999999, "confirmed", repo=repo)


class TestReassignIdentifier:
    def test_reassigns(self, db, repo):
        p1 = _insert_person(db, "A", "A")
        p2 = _insert_person(db, "B", "B")
        add_identifier(db, p1, "orcid", "0000-0001", repo=repo)
        db.execute("SELECT id FROM person_identifiers WHERE id_value='0000-0001'")
        ...
        reassign_identifier(db, ident_id, p2, repo=repo)
    ...
    def test_raises_not_found(self, db, repo):
        p = _insert_person(db)
        with pytest.raises(NotFoundError):
            reassign_identifier(db, 999999, p, repo=repo)


class TestSetRejected:
    def test_marks_rejected(self, db, repo):
        p = _insert_person(db)
        set_rejected(db, p, True, repo=repo)
        db.execute("SELECT rejected FROM persons WHERE id = %s", (p,))
        assert db.fetchone()["rejected"] is True

    def test_unmarks(self, db, repo):
        p = _insert_person(db)
        set_rejected(db, p, True, repo=repo)
        set_rejected(db, p, False, repo=repo)
        db.execute("SELECT rejected FROM persons WHERE id = %s", (p,))
        assert db.fetchone()["rejected"] is False

    def test_raises_not_found(self, db, repo):
        with pytest.raises(NotFoundError):
            set_rejected(db, 999999, True, repo=repo)


class TestUpdateName:
    def test_updates_name_and_refreshes_forms(self, db, repo):
        p = create_person(db, "Dupont", "Jean", repo=repo)
        ...
        update_name(db, p, "Martin", "Sophie", repo=repo)
    ...
    def test_raises_not_found(self, db, repo):
        with pytest.raises(NotFoundError):
            update_name(db, 999999, "X", "X", repo=repo)
...
class TestBatchAssignOrphanAuthorships:
    ...
    def test_empty_list_returns_zero(self, db, repo):
        self._setup_uca(db)
        person_id = _insert_person(db)
        assert batch_assign_orphan_authorships(db, person_id, [], repo=repo) == 0

    def test_assigns_and_creates_truth(self, db, repo):
        self._setup_uca(db)
        person_id = _insert_person(db)
        pub_id = _insert_publication(db)
        ...
        assigned = batch_assign_orphan_authorships(db, person_id, [sa1, sa2], repo=repo)
    ...
    def test_skips_already_assigned(self, db, repo):
        self._setup_uca(db)
        p1 = _insert_person(db, "A", "A")
        p2 = _insert_person(db, "B", "B")
        pub_id = _insert_publication(db)
        ...
        assigned = batch_assign_orphan_authorships(db, p2, [sa1], repo=repo)
...
class TestDetachAuthorships:
    def test_detaches_and_removes_truth_if_orphan(self, db, repo, authorship_repo):
        person_id = _insert_person(db)
        pub_id = _insert_publication(db)
        sp_id = _insert_source_publication(db, pub_id)
        sp_person = _insert_source_person(db)
    ...
    def test_cleans_name_form_when_no_remaining(self, db, repo, authorship_repo):
        person_id = create_person(db, "Dupont", "Jean", repo=repo)
    ...
    def test_keeps_name_form_if_another_authorship_uses_it(self, db, repo, authorship_repo):
        person_id = create_person(db, "Dupont", "Jean", repo=repo)
        pub_id = _insert_publication(db)
        sp_id = _insert_source_publication(db, pub_id)
        sp_person = _insert_source_person(db)
...
class TestDetachNameForm:
    def test_removes_person_from_form(self, db, repo):
        p1 = create_person(db, "Dupont", "Jean", repo=repo)
        p2 = create_person(db, "Dupont", "Jean", repo=repo)  # même forme 'dupont jean'

        detach_name_form(db, p1, "dupont jean", repo=repo)
    ...
    def test_deletes_form_when_last_person_detached(self, db, repo):
        person_id = create_person(db, "Unique", "Name", repo=repo)

        detach_name_form(db, person_id, "name unique", repo=repo)
...
class TestAssignOrphanAuthorship:
    ...
    def test_raises_on_invalid_source(self, db, repo):
        with pytest.raises(ValidationError, match="Source inconnue"):
            assign_orphan_authorship(db, 1, "invalid", 1, repo=repo)

    def test_returns_false_if_already_assigned(self, db, repo):
        ...
        assert assign_orphan_authorship(db, person_id, "hal", sa_id, repo=repo) is False

    def test_assigns_and_creates_truth_authorship(self, db, repo):
        self._setup(db)
        person_id = _insert_person(db)
        pub_id = _insert_publication(db)
        sp_id = _insert_source_publication(db, pub_id)
        ...
        result = assign_orphan_authorship(db, person_id, "hal", sa_id, repo=repo)
    ...
    def test_skips_name_form_if_excluded(self, db, repo):
        ...
        assign_orphan_authorship(db, person_id, "hal", sa_id, repo=repo)
