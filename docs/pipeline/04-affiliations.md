...
3. **`populate_affiliations`** — rafraîchit la vue matérialisée `source_authorship_structures`, qui rattache chaque signature aux structures de ses adresses. Le drapeau `in_perimeter` des `source_authorships` se déduit de cette vue : une signature est `in_perimeter` dès qu'une de ses structures appartient au périmètre. Un document sans publication dont une signature entre dans le périmètre est signalé à la phase [`publications`](07-publications.md), qui le met en publication. Code : `application/pipeline/affiliations/populate_affiliations.py`.
