...
3. **`check_journals_in_sudoc`** — vérifie les ISSN des revues dans le [Sudoc](../sources/09-sources-supplementaires.md#sudoc). Sont reprises les revues jamais vérifiées (`sudoc_checked_at` nul) qui portent un ISSN, valide ou rejeté. Le Sudoc donne l'ISSN-L et le support de chacun de leurs ISSN. L'ISSN-L majoritaire devient `issnl`, et un ISSN d'un autre ISSN-L est retiré. Un ISSN rejeté (`rejected_issns`) est corrigé quand une faute de frappe près mène à une notice de même ISSN-L et de titre très proche. Chaque ISSN est ensuite rangé dans la colonne de son support : `issn` pour le papier, `eissn` pour l'électronique. Une revue qui reçoit un ISSN nouveau redevient à vérifier. Les revues sont vérifiées 4 à la fois, à 5 requêtes par seconde au plus toutes revues confondues. Une revue dont une requête échoue reste à vérifier, et le run suivant la reprend.
