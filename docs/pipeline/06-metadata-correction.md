...
1. **`correct_unary`** — mappe le type de document de la source vers le vocabulaire unifié, et la langue vers le code du référentiel `languages` d'après les formes de `language_forms`. Une langue absente du référentiel, comme `und`, devient NULL. La passe applique ensuite les règles de correction qui s'appliquent à un enregistrement isolé. Ces règles corrigent le type de document, le statut *open access* et les identifiants associés au document.
...
2. **`correct_by_cluster`** — rapproche les `source_publications` partageant un même DOI et déduit le DOI que doit porter chaque membre du groupe.
