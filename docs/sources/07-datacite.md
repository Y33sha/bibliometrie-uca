...
DataCite est intégré en **DOI-driven uniquement** : on l'interroge pour enrichir les DOI déjà connus, et pour résoudre concept/version et les préfixes. Voir la fiche chantier [`METIER_doi-ra-datacite`](https://github.com/Y33sha/bibliometrie-uca/blob/master/docs/chantiers/METIER_doi-ra-datacite.md).
...
- Relations entre publications au-delà du concept/version (supplément ↔ article, chapitre ↔ ouvrage…) : la matière est ingérée et conservée, sa modélisation relève du chantier `METIER_relations-publications`.
