...
**Sources API** (https://api.openalex.org/sources) — enrichissement sur les montants [APC](../glossaire.md#apc) des revues, en toute fin de pipeline ([phase `enrich`](../pipeline/08-enrichissements.md#oa_status)).
Récupération des prix APC catalogue par `openalex_id` de la revue.
Met à jour `journals.apc_amount`, `apc_currency`, `is_in_doaj`.
> Informations non utilisées dans l'appli: pas sûr qu'on garde cette étape.
...
- Les ORCIDs récupérés via `author.orcid` sont ceux de l'**entité auteur unifiée** par OpenAlex, et ne figurent pas nécessairement dans la publication. Ils sont stockés tels quels dans `source_authorships.person_identifiers`, puis filtrés côté pipeline persons par [`keep_orcid_if_name_matches`](https://github.com/Y33sha/bibliometrie-uca/blob/master/domain/sources/openalex.py) : l'ORCID n'est utilisé pour rattacher la signature à une `person` que si le `display_name` de l'entité auteur OA est compatible avec le `raw_author_name` de la signature.
