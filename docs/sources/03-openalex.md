...
- Les requêtes API paginées tronquent les authorships à **100 auteurs max** par publication ; ces works sont marqués `staging.authors_truncated` à l'extraction, et [refetch_truncated](https://github.com/Y33sha/bibliometrie-uca/blob/master/infrastructure/sources/openalex/refetch_truncated.py) les re-télécharge individuellement pour récupérer la liste complète.
