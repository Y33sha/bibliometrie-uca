...
**Sources API** (https://api.openalex.org/sources) — enrichissement par `openalex_id` de la revue. Sub-step `enrich_journals_from_openalex` de la phase [`publishers_journals`](../pipeline/04-publishers-journals.md). Met à jour `journals.apc_amount`, `apc_currency`, `is_in_doaj` (flag), `journal_type`.
