...
Vu l'hétérogénéité des entités personnes selon les sources, il a été décidé de ne pas maintenir de table `source_persons`. Les informations récupérées depuis les sources (forme de nom, identifiants éventuels) sont portées par `source_authorships` (`raw_author_name` pour traçabilité) et par son identité d'auteur dédupliquée `author_identifying_keys`, reliée par `identity_id` (`author_name_normalized` pour le matching par nom, `person_identifiers` JSONB pour les identifiants persistants — ORCID, IdRef, idHAL). La déduplication / création des personnes canoniques se fait dans la [phase `persons`](../pipeline/09-persons.md) du pipeline à partir de ces éléments.
