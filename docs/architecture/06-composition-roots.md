...
Le composition root est l'endroit où les adaptateurs concrets sont **instanciés** et **câblés** aux use-cases. Lui seul a le droit d'importer `infrastructure.*` directement. Partout ailleurs, on reçoit un port en paramètre.
...
- `interfaces/cli/run_pipeline.py` — orchestrateur pipeline complet ; ses wrappers `_run_*` sont les composition roots de chaque phase (ouverture de connexion, instanciation des adaptateurs, appel de l'orchestrateur applicatif)
