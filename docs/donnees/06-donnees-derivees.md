...
Le mode incrémental fait l'hypothèse que l'état antérieur est correct. Une évolution des règles en amont peut donc laisser un **drift** : des dérivés figés sur l'ancienne logique, jamais re-marqués. Plusieurs traitements offrent pour cela un recalcul complet de récupération — par exemple `run_pipeline.py --only publications --rebuild-publications` (re-marque tout le stock `dirty` avant de le traiter) ou `build_authorships --rebuild-full` (`TRUNCATE` + reconstruction). À lancer après un changement de règles, pour matérialiser ce que le mode incrémental ne verrait pas.
