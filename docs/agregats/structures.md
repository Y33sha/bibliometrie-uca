...
**Formes de nom** (`POST`, `PUT`, `DELETE /api/name-forms`). Le texte passe par la même normalisation que les adresses auxquelles il sera confronté.

Une forme de six caractères ou moins doit exiger une frontière de mot : cherchée en sous-chaîne, elle serait reconnue à l'intérieur d'autres mots — « uca » dans « éducation ». La règle est imposée d'office, et une contrainte de base la verrouille.
