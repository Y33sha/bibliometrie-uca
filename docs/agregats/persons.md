...
2. **Arbitrer les identifiants disputés.** Un ORCID, un IdRef ou un identifiant de compte HAL est attribué à une personne, alors que des signatures qui le portent sont rattachées à une autre : c'est le conflit. Le nom d'auteur majoritaire, parmi toutes les signatures portant cette valeur, tranche. L'identifiant ne change de titulaire que si ce nom désigne la personne concurrente et non la titulaire en place ; un partage indécis ne déplace rien, et une attribution confirmée par la curation reste intouchée. Après un transfert, les signatures qui tenaient leur rattachement de cet identifiant repassent à nul et sont résolues à nouveau.
