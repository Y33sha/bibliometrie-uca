...
4. **Deux voies d'écriture de `journal_type`, requalification asymétrique.** Le pipeline (`enrich_journals_from_openalex`) écrit `journal_type` sans requalifier les publications ; l'API (`commands.update_journal`) déclenche `requalify_publications_for_journal`. Une revue typée par le pipeline peut laisser des `doc_type` incohérents jusqu'au prochain `refresh_from_sources` — le CLI `refresh_publications_for_journal_type.py` sert de rattrapage.
5. **Champs éditables définis en double.** `JournalUpdateFields` (port, TypedDict) et `JournalUpdate` (modèle Pydantic API) listent quasiment les mêmes champs, à maintenir en phase à la main.
6. **`merge_journal_into` : SQL cross-agrégat et contrainte gérée à la main.** La fusion mêle SQLAlchemy Core et `text()` pour écrire `publications` / `source_publications` / `apc_payments` (tables hors MetaData du repo journal), et neutralise l'`openalex_id` source avant le COALESCE pour éviter une violation d'unicité — logique sensible à l'ordre des statements.
7. **Cascade `find_or_create_journal` incohérente entre branches.** Les chemins openalex / ISSN réenregistrent une forme de nom via `enrich_journal`, mais le chemin par titre ne le fait pas ; un `# TODO` explicite subsiste dans `core.py`.
...
- **Cohérence `journal_type` → `doc_type`.** Le `doc_type` des publications d'une revue doit rester cohérent avec le `journal_type` de celle-ci ; `requalify_publications_for_journal` maintient la règle. Elle n'est déclenchée que sur la voie API, pas sur la voie pipeline (cf. point d'attention 4) : invariant réel mais inégalement appliqué.

La minceur de cette section confirme le caractère anémique de l'agrégat : hors la requalification, aucune règle transverse n'est portée par le concept `Journal`.
