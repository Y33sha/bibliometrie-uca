...
- [x] **4a. DOI ↔ journal** : matérialisé sous forme de playbook diagnostic + remédiation manuelle plutôt que d'un affichage frontend automatisé. Cf. [`playbooks/nettoyer-journals-publishers.md`](../playbooks/nettoyer-journals-publishers.md) (Cas 5 : query d'audit ; Cas 1-4 : pré-requis de nettoyage pour que l'audit soit lisible). `journals.doi_prefix` seedé via `interfaces/cli/oneshot/seed_journals_doi_prefix.py` (filtrage outliers preprint, guard ISBN-13, threshold 1 char après `/`).
...
- [x] **4d. Sujets ↔ revue** : reporté vers [METIER_sujets-qualite](METIER_sujets-qualite.md) (chantier dédié regroupant les contrôles qualité/cohérence des sujets, dont le contrôle sujet ↔ revue).
