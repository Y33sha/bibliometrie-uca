...
2. **Restreindre le matching aux identifiants `confirmed`** — option de configuration, `false` par défaut. Les identifiants `pending` (ajoutés automatiquement par le pipeline, non vérifiés) restent no-op tant que non confirmés. Sur une base vide sans identifiants vérifiés, c'est un changement de comportement trop radical (les personnes découvertes par le pipeline n'ont que des `pending` et deviennent non-matchables par identifiant) ; sur une base déjà fournie et vérifiée à la main, c'est une option conservatrice raisonnable. D'où le défaut `false` et l'activation décidée par l'utilisatrice.
...
- [x] **Corroboration par le nom au matching identifiant** (9d93580a) : un match par ORCID / hal_person_id / idref dont le nom de la personne ciblée est incompatible avec la signature (`names_compatible`) est refusé, chaque rejet journalisé (identifiant + les deux formes). Reste à statuer sur les faux rejets résiduels (changement de nom, translittération).
- [x] **Détachement par identifiant** (71ccec83) : détache rétrospectivement l'intrus tenu par un identifiant quand la même `source_publication` porte aussi une signature au nom compatible (l'ancre) — ~11,9k rattachements / 104 personnes, à 99 % via ORCID crossref + openalex.
- [x] **Généraliser au départage par statut de forme de nom** : dans un groupe `(source_publication, personne)` à ≥2 signatures, l'occurrence compatible avec une forme **`confirmed`** est l'ancre ; celle qui ne correspond qu'à une forme **`pending`** incompatible est un intrus étranger (un vrai intrus est porté par une forme pending — « i perez rafols » collé à Ravoux par une méga-publi à identifiant partagé). Détacher l'intrus ⇔ rejeter sa forme (verrou étape 2). Comparer au nom canonique seul est trop grossier (rejette à tort « Pm Llorca », initiales collées d'une forme confirmée, et « S. Porteboeuf », nom composé). Trois cas : **détachable** (ancre + intrus), **doublon de signature** (toutes les occurrences légitimes : duplication source / désalignement de positions des méga-papers, problème distinct), **sans ancre confirmée** (à examiner). Faux positif résiduel du bucket détachable : le **nom marié non encore confirmé** (« Sarah Julie Porteboeuf » pour une « Porteboeuf-Houssais ») — à lever par confirmation humaine, pas en aveugle. D'où une **résolution via l'UI de l'étape 4** (suspects + confirm/reject) avant tout détachement automatique — file « Intrus détachables » du hub `admin/persons`, adossée à l'index partiel `idx_sa_pub_person` (df0c5ed4).

### Étape 2 — Verrouiller le non-retour

- [x] **Verdict bloquant `name_form ↔ person`** (d88a9028, cb0f859a) — antidote à l'amplificateur secondaire (une signature mal rattachée devient une forme de nom de la personne et en attire d'autres par matching de nom ; verrouille ce que l'étape 1 vient de détacher). Statut `pending` / `confirmed` / `rejected` sur `person_name_forms` (enum partagée avec `person_identifiers`). Les formes dérivées du nom/prénom (source `'persons'`) sont confirmées d'office. Le recalcul (populate et chemin live) ne touche jamais aux statuts et ne supprime jamais un `confirmed`/`rejected`, sauf une forme `'persons'` rendue obsolète par une édition du nom. Au matching : `fetch_name_form_map` exclut les `rejected` (la forme rejetée ne propose plus la personne par nom) ; la corroboration du match par identifiant consulte le statut — `confirmed` corrobore sans test de tokens (gère le changement de nom), `rejected` refuse, sinon test de compatibilité par tokens. Les actions humaines confirm/reject (endpoints + UI) sont en étape 4.
...
- [x] **Sources de paires candidates** : les trois sont des files de triage du hub — (i) **formes de nom ambiguës**, (ii) **conflits d'identifiant** (même valeur d'identifiant brut sur des `person_id` distincts), (iii) **doublons par nom** (paires aux noms compatibles, `PERSON_DUP_QUERIES` + `names_compatible`). Les conflits d'identifiant ne sont pas une machinerie à part : une source de paires de plus, à fort signal.
- [x] **Classification homonymie / doublon / erreur** : recouvrements chiffrés affichés par paire (co-auteurs, publications co-signées, labos, revues) et tri par force de réseau décroissante — réseaux communs en tête (doublon), disjoints en fin (homonyme). Décision empirique : **pas de score pondéré a priori** ni de pastille — les chiffres et le tri suffisent à l'œil ; les **sujets sont écartés** (trop larges pour discriminer).
- [x] **Verdicts sur les formes de nom ambiguës** : la file « Formes ambiguës » matérialise la classification en statut `person_name_forms` via confirm / reject inline (homonymie → garder ; doublon → fusion ; erreur → `rejected`, verrou de l'étape 2).
- [x] **Forme de l'outillage** : *evidence-forward* dans le hub — éléments de décision chiffrés sous les yeux, action inline. Pas de correction automatique : les erreurs nettes restent à l'œil humain, l'outillage les surface et les rend actionnables d'un clic.
...
- [x] **Liste maîtresse + facets** : liste person-centrée non filtrée, recherche, facets par catégorie (département / rôle / base RH / identifiants présents) et facette « À confirmer » (formes de nom et identifiants au statut `pending` — caa6d984) ; ajout manuel d'identifiant via le drawer.
- [x] **Drawer-personne** (`PersonDrawer.svelte`) : panneau latéral ouvert sur `?person=:id` (deep-link), où le clic sur une personne mène désormais — plus vers la page publique `persons/id`. Porte identité, identifiants (+ statuts), formes de nom, fusion ; la liste maîtresse est allégée en conséquence (nom, compteurs, RH). Reste : option pleine page, et la résolution deep-link d'un id hors page courante (fetch).
- [x] **Section formes de nom du drawer** (`NameFormsList.svelte`) : **toutes** les formes (dont `persons`-seules, désormais remontées par la projection), puce de statut confirmed / pending / rejected et boutons confirm / reject inline (style commun avec les identifiants via `.status-chip` / `.status-btn`), pointeur « partagée avec N » (`shared_count`), formes canoniques (`'persons'`) verrouillées. Le clic sur la forme ouvre toujours la modale détaillée (publications + détachement). `NameFormSummaryOut` enrichi (`status`, `shared_count`).
- [x] **Endpoints confirm / reject de formes** + bascule de `detach_name_form` (08e7cf92) : `PATCH /api/persons/{id}/name-forms/status` (primitive `update_name_form_status`, audit `person_name_form.status_changed`) ; l'ancien détachement par DELETE est remplacé par un statut `rejected` durable (bloque le retour au matching par nom ; cf. verrou étape 2). La modale existante repointe sur l'endpoint.
- [x] **Onglets-files de triage** unifiés sous le hub (replier `admin/person-duplicates`), une file par détecteur, décision *evidence-forward*, action inline sur la ligne. Première file en place : **« Conflits d'identifiant »** (paires de `person_id` au même ORCID / IdRef / hal_person_id / idHAL, hors `_dubious`), evidence-forward (noms, labos, nb publications, identifiant partagé) avec fusion / marquage distinct inline et ouverture du drawer. Adossée à une **matview `person_identifier_keys`** (substrat de scinder Phase 0, rafraîchie en phase `persons`) : la détection passe de ~25 s (scan des 16 M `source_authorships`) à instantané. ~89 paires au stock. Les files **« Formes ambiguës »**, **« Intrus détachables »** (personnes rattachées à ≥2 signatures d'une même publication, départage ancre/intrus par statut de forme, adossée à l'index partiel `idx_sa_pub_person` — df0c5ed4) et **« Doublons par nom »** (paires aux noms compatibles triées par recouvrement de réseau — 0adcb7ab) sont aussi en place. `admin/person-duplicates` est entièrement replié et **supprimé** (560267ca) : page, query, port, router, détecteur conflit-de-sources enterré. Le compteur « Doublons par nom » est paresseux (4 self-joins ~3 s) ; optimisation matview possible si besoin.
- [x] **Garde-fou de fusion plutôt que throughput** : navigation clavier, *undo* et actions *bulk* abandonnés — l'action mutant la liste rend le clavier peu utile, les décisions se font au cas par cas (pas de lot), et l'*undo* est trivial sur les actions réversibles mais très coûteux sur la fusion (réassignation de FK sans mémoire de la paternité des authorships). À la place, une **confirmation avant fusion** signalant le nombre de publications réattribuées et l'irréversibilité, sur tous les points d'entrée du hub (d287b306).
- [x] **Audit trail** : chaque confirm / reject / detach / merge émet un événement (`emit_event`) enregistré dans `audit_log`, l'utilisateur admin étant posé par `auth_middleware` sur chaque écriture (chemin router → command → core déjà en place ; couverture bout-en-bout verrouillée — e93cc349).

### Renforts optionnels (hors chemin critique)

Conservateurs, activables une fois le reste en place ; pas des prérequis.

- [ ] **Option de config `confirmed`-only** (défaut `false`) : restreint le matching aux identifiants `confirmed` ; les `pending` deviennent no-op. Évaluer l'impact (volume d'orphelins, besoin d'un flux de confirmation) avant d'envisager de l'activer sur une base vérifiée.
...
* [x] générer une liste de suggestions de fusions (conflit d'identifiants entre 2 `person_id`) — file « Conflits d'identifiant » du hub `admin/persons`, adossée à la matview `person_identifier_keys` (cf. étape 4).
