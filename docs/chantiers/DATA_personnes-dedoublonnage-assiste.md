...
- [x] **Liste maîtresse + facets** : liste person-centrée non filtrée, recherche, facets par catégorie (département / rôle / base RH / identifiants présents) et facette « À confirmer » (formes de nom et identifiants au statut `pending` — 7a04491f) ; ajout manuel d'identifiant via le drawer.
...
* [x] repenser entièrement la page `admin/person-duplicates` — **supprimée** (e707e75f), remplacée par les files de triage du hub `admin/persons`. (`admin/duplicates`, dédoublonnage des publications, reste inchangée.)
