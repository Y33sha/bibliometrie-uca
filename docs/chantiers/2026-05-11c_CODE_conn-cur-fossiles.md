...
### Phase 5 — Audit des Protocols

L'audit "fossile dans une méthode de Protocol" demande de
cross-référencer avec les implémentations concrètes (une signature
de Protocol est fossile uniquement si **aucune** implémentation
n'utilise le paramètre dans son corps).

- [x] Script AST : pour chaque méthode de Protocol dans
  `application/ports/*` et `domain/ports/*` qui déclare `conn`/`cur`
  en 1er arg, vérifier ses implémentations dans `infrastructure/`.
- [x] Résultat : **0 fossile**. 111 méthodes de Protocol scannées
  (105 noms uniques), toutes ont au moins une implémentation qui
  utilise effectivement l'argument. Les signatures sont saines.
