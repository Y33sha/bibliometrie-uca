...
- [ ] **Suppression du statut de run** — `logs/status.json` porte l'avancement du run en cours, écrit par l'orchestrateur et relu par la page d'administration du pipeline en interrogation rapprochée. Le fichier oblige à ouvrir un second emplacement en écriture pour un affichage que personne ne regarde quand le pipeline tourne la nuit. Le supprimer touche l'orchestrateur, l'endpoint de statut, son modèle, et le bandeau de la page qui l'interroge ; l'historique des runs, servi en base, ne bouge pas.
