...
- [x] `structure_relations.relation_type` (colonne texte, deux valeurs endogènes `est_tutelle_de` / `est_partenaire_de`) reçoit un StrEnum `StructureRelationType` (`domain/structures/relations.py`), référencé par `structures.py` et `perimeter.py`. L'écriture est validée dans le service `create_relation` via `require_known_relation_type` (même patron que `require_known_source`) : le vocabulaire contraint effectivement les écritures, au niveau de l'application. La colonne reste `text` (pas de `CHECK` ni d'enum PostgreSQL — même situation que `id_type`) : un durcissement base est possible pour les deux colonnes texte à validation applicative.
