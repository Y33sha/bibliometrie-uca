...
- [x] ISSN dans la table `journal_issns` (chantier [Table des ISSN](2026-09-22_DATA_table-issn.md)). Un même ISSN peut figurer dans plusieurs revues ; les règles de fusion résolvent le partage, et l'onglet « ISSN partagés » montre le reste.
...
- [x] The Conversation : Crossref dépose ses articles sans titre de revue, et 277 publications restent sans revue après le retrait. Le type de document se déduit du déposant : l'éditeur du préfixe DOI peut porter le type `media` (migration), et la règle `DOI_REGISTRANT_MEDIA_TO_MEDIA` type ses documents en `media`. « The Conversation Media Group » est à typer `media` dans l'administration. Le déposant porte deux revues (France, Espagne). L'espace de noms DOI `10.64628/aak.`, appris des dépôts HAL qui nomment *The Conversation France*, rattache ses articles à cette revue (chantier [Revues divergentes](../DATA_revues-divergentes-entre-sources.md), phase 6).
- [x] Retrait de la colonne `journals.doi_prefix` (migration), de `seed_journals_doi_prefix` et de leurs usages : administration, page publique d'une revue, playbook de nettoyage. Une revue connaît plusieurs préfixes au fil des changements d'éditeur et de convention : une colonne unique ne les décrit pas. La table `journal_doi_namespaces` les décrit (chantier [Revues divergentes](../DATA_revues-divergentes-entre-sources.md), phase 6).
...
- **Unicité des ISSN.** Un ISSN désigne-t-il une seule revue ? Si oui, l'ordre de rapprochement de `find_or_create_journal`, qui essaie l'`openalex_id` avant les ISSN, est à réexaminer.
