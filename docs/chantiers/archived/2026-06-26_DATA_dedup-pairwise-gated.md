...
- [x] **Tier 1 — matérialiser** (fait) : token `metadata_block = (doc_type, title_normalized, pub_year)`, gardé par `length(title_normalized) > 30`, dans `domain/source_publications/keys.py` + branche de l'univers de `reconcile_components`. Pas de garde auteurs. **Généralisé à tous les `doc_type`** (pas seulement les types sans identifiant fiable) : l'audit donne ~99 % de même-œuvre par type au-delà du seuil, et le cannot-link DOI protège les types identifiés d'une fusion erronée (deux DOI distincts ne co-clusterisent jamais, même co-bloqués sur le titre). Le token thèse `(title, year)` est **absorbé** dans ce token (`thesis|<titre>|<année>`). Tests `test_keys` + `test_reconcile_components` ; rollout sur le stock via l'oneshot `backfill_title_normalized` (re-dirtie les types à token).
...
- **Divergence Python ↔ SQL des tokens (gardée par test, élimination possible).** Le critère d'égalité de chaque token est encodé deux fois : l'**extraction** Python (`project_confirmation_keys`, pour le clustering en mémoire) et le **fetch du voisinage** par branche `UNION` dans `_UNIVERSE_SQL` (le JOIN d'égalité). Les deux doivent rester synchrones — le plus à risque étant le seuil de longueur `metadata_block`, dupliqué des deux côtés. Garde en place : un **test différentiel** (`test_reconcile_components`) qui vérifie que le voisinage SQL relie exactement les SP que les tokens Python relient (jeu couvrant chaque type + les bords de garde). Élimination structurelle possible : **matérialiser les tokens en projection SQL** émettant `(sp_id, namespace, value)` — l'univers devient alors un self-join générique (une seule branche, plus une par token), et le clustering Python consomme les tokens émis au lieu de les ré-extraire → une seule définition, divergence impossible, et « ajouter un token » = un `SELECT`. Coût : retirer l'extraction Python du chemin pipeline. Petit chantier CODE à part.
