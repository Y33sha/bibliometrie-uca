...
- [x] `refresh_from_sources` → recompute pur : retirer le `find_by_doi` + l'auto-merge (`merge_publications`). Il ne connaît plus que les `source_publications` de sa propre publication. (`02ff6d43`)
- [x] `merge_publications` = `merge_into` puis recompute depuis les sources, au lieu de `Publication.absorb`. Supprime `Publication.absorb` (agrégation métadonnées) et `absorb_oa_status` (`domain/publications/metadata`), lève la divergence OA, et — l'auto-merge de `refresh` étant parti — sans circularité `merge → refresh → merge`. (`2ea9aa5f`)
- [x] Reformuler le commentaire d'ordonnancement de `reconcile` (étape 2, dissolutions) : la raison devient « libérer le DOI avant que le survivant le reprenne », plus « éviter que l'auto-merge se déclenche ». (`2ea9aa5f`)
