...
- [x] Run complet du pipeline. Un crash a révélé un bug d'alignement du pré-merge DOI dans `refresh_from_sources` (le pré-merge cherchait le DOI brut, `merge_source_rows` posait le DOI normalisé via VO `DOI` qui strip les suffixes `.vN`). Corrigé en commit `9221df7`. Run redémarré et complété sans crash.
