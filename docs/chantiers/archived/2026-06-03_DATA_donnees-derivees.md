...
Commencé le 2026-06-01 - Terminé le 2026-06-03
...
**Deuxième garde de faisabilité — identité stable et mutation per-row.** Même *purement content-dérivé*, un artefact ne peut pas devenir un matview si son `id` est une **identité durable** (cible de FK — Postgres l'interdit vers un matview ; ou handle tenu par le frontend/l'API qui doit rester valide entre deux refresh) ou s'il est **muté row par row en temps réel** (un matview est read-only → chaque écriture unitaire deviendrait un `REFRESH` complet). Cas net : `authorships` (cf. Phase 2) — content-dérivée depuis l'extraction du sidecar, mais `id` cible de FK + handle frontend/API + exclusion/propagation per-row. Le contenu dérivable ne suffit pas ; l'identité et le mode d'écriture comptent.

Ce sont des critères de **faisabilité**, pas de perf : ils éliminent d'office certaines conversions.
...
| `authorships` (table) | rebuild consolidé. **Natif dissous** (migration `f3b6d9c1a8e2`) : `source_manual` (vestigial) droppé, `excluded` extrait en store `rejected_authorships(publication_id, person_id)`. Content-dérivée, mais **matview rejeté** (cf. Phase 2) : `id` cible de FK + handle externe durable (frontend/API) + mutations per-row temps-réel. **Garder table maintenue** ; chemin chaud + consolidation |
| `persons` (table) | matching identités + **état natif** (`rejected`, fusions via `distinct_persons`) |
| `source_authorship_structures` | **JOIN pur** `source_authorship_addresses ⋈ address_structures` (confirmées) — le matching adresse→structure produit `address_structures` en *amont*, ce n'est pas la dérivation ici. Feuille, sans état natif → matview *possible* en principe. Mais **pas pertinent en l'état** : la dérivation filtre par la **clôture récursive du périmètre** (config `perimeter_affiliations`), elle est **en amont du matview `authorship_structures`** (matview-sur-matview), et le chemin admin fait déjà un **resync chirurgical par SA**. À **subordonner à la matérialisation du périmètre** ([`DATA_perimeter-materialise`](DATA_perimeter-materialise.md)) : si `perimeter_structures` est matérialisé, la définition se réduit à un JOIN et la question redevient propre |
...
- [x] **`authorship_structures` en matview** (migration `a2c6e4f8b1d7`). Benchmark prod : `REFRESH` full ≈ **2 s** (`SELECT DISTINCT sa.authorship_id, sas.structure_id … WHERE authorship_id IS NOT NULL` ; le filtre `v_active_publications` du build était redondant et coûteux — 482 k nested loops, 21 s → 2 s sans). Perf-lecture neutre. Bonus correction : l'incrémental add-only laissait ~215 liens obsolètes en prod (la matview est exacte à chaque refresh). Code de propagation impératif retiré (`propagate_perimeter_and_structures_from` → `propagate_perimeter_from`, `reset…and_structures` → `reset…perimeter`, `recompute…and_structures` → `recompute…in_perimeter`, blocs INSERT/DELETE/TRUNCATE supprimés). `REFRESH … CONCURRENTLY` en fin de phase `authorships` (build) **et** sur les 3 chemins admin (review adresse↔structure, assign orphelin single/batch), ~2 s/action — **à surveiller : candidat `CODE_background-jobs` si gênant à l'usage**. La matview n'a pas de FK : le `ON DELETE CASCADE` d'`authorship_id` devient un nettoyage au refresh (inerte entre-temps, lectures inner-join `authorships`).
- [x] **`authorships` en matview — rejeté, garder table maintenue.** Le sidecar (fait) rendait la table *content-dérivée*, mais trois propriétés la disqualifient, indépendantes de la perf-lecture : (1) **cible de FK** — `source_authorships.authorship_id REFERENCES authorships(id)`, et Postgres interdit une FK vers un matview ; (2) **identité externe durable** — `authorships.id` est un handle tenu par le frontend (`PublicationsListView` → `onExcludeAuthorship(authorship_id)`) et l'API (`PATCH /api/authorships/{id}/exclude`, `delete_authorship`), qu'un refresh régénérerait (`row_number`) ; (3) **mutation per-row temps-réel** — `exclude_authorship` (DELETE par id), `propagate_uca_for_addresses` / `propagate_in_perimeter_to_authorships` (UPDATE ciblés après review d'adresse) écrivent à l'unité, là où un matview read-only imposerait un `REFRESH` complet (>100 k rows) par clic admin. Le seul bénéfice qu'un matview aurait apporté — pruning automatique des orphelines — est obtenu par **un `DELETE`** : l'incrémental `build_authorships` est add-only, donc une authorship dont plus aucune source n'atteste la paire survit jusqu'au rebuild `full` mensuel ; on généralise `delete_orphan_authorships_for_person` en prune global câblé après le linking FK (tourne aussi en incrémental).
- [x] [`DATA_perimeter-materialise`](DATA_perimeter-materialise.md) — réactivable comme sous-chantier si l'audit valide la matérialisation de `perimeter_structures` et/ou la suppression de `in_perimeter`
