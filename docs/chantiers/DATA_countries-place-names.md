...
- [x] **Pivot ROR pour les institutions** : seed **autoritaire** depuis le registre ROR plutôt qu'empirique. Oneshot `seed_place_names_from_ror` (`--build` télécharge le dump v2.8 épinglé → aplatit `names × country_code`, mono-pays, **hors acronymes** car FP sur mots courants → `data/ror_institutions.csv` gitignoré ; défaut = insert). **191 246** noms complets mono-pays. Remplace les fournées empiriques : migration `c8f3a6b1e4d7` **vide les formes non-country** avant re-seed (les anciennes migrations de seed restent dans l'historique). Restent à faire : les **acronymes** (cnrs/mit… en 2e temps, filtre à trouver) et le **prune** des formes ROR non-matchées après détection.
