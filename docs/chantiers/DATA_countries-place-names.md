...
- [x] **Pivot ROR pour les institutions** : seed **autoritaire** depuis le registre ROR plutôt qu'empirique. Oneshot `seed_place_names_from_ror` (`--build` télécharge le dump v2.8 épinglé → aplatit `names × country_code`, mono-pays, **hors acronymes / numériques / < 6 car.** car FP → `data/ror_institutions.csv` gitignoré, 190 205 formes ; défaut = insert). Remplace les fournées empiriques : migration `c8f3a6b1e4d7` **vide les formes non-country** avant re-seed (anciennes migrations de seed conservées dans l'historique).
- [x] **Prune** (`prune_place_names`) : scanne **toutes** les adresses (pas que les sans-pays — les formes présentes dans des adresses déjà résolues resservent au full rerun), supprime les formes hors corpus (0 occurrence). 191k → **24 017 institutions** pertinentes. Nettoyage des formes numériques/courtes qui avaient pollué `countries` : 129 adresses remises à NULL + dirty.
- [x] **Récupération du delta empirique** : les formes empiriques **absentes de ROR**, curées en plusieurs passes — soustraction brute, exclusion des formes incluant une forme ROR même-pays (`university of toronto 25` ⊂ `university of toronto`), dédup par inclusion (garde la courte), rognage des **mots de bord génériques** (connecteurs + champs/structure : `biology`, `department`, `of`…), puis curation manuelle — ~**1090 réintégrées** via `seed_place_names_from_ror --csv`.
- [ ] **Acronymes** ROR (cnrs/mit/cern… valent le coup) en 2e temps — filtre à trouver pour écarter les mots courants (`set`/`art`…).
- [ ] Marches suivantes **empiriques** (hors ROR) : **villes** (`kind = 'city'`) + **codes postaux** (`kind = 'postal_code'`) — candidats singleton-pays hors France dans `place-name-candidates.tsv`.
