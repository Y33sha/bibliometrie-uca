...
- [ ] **`/api/laboratories/{id}` sert n'importe quelle structure.** La liste filtre sur le périmètre `persons` et sur les types que porte `laboratories_display_types` ; le détail, les adresses, les sujets et le tableau de bord ne filtrent rien. L'identifiant d'une université ou d'un site rend donc 200 sous le titre « profil public d'un laboratoire », et son 404 ne tombe que sur une structure inexistante. Le frontend n'y mène pas — il navigue depuis la liste — mais le contrat annonce un laboratoire et sert une structure. À trancher : le détail filtre comme la liste, ou l'endpoint assume de servir toute structure du périmètre.
