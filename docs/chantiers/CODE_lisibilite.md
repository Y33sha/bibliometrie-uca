...
- [ ] `persons_directory` et `list_persons` (`queries/api/persons/list.py`) émettent la même requête de comptage, au caractère près. Leurs projections, elles, diffèrent à bon droit : l'annuaire public rend une vue réduite quand la liste admin expose noms normalisés, dates, `rejected`, statuts d'identifiants et formes de nom.
