...
- [ ] Rejouabilité au niveau canonique rendue vérifiable plutôt que documentée. Aujourd'hui la frontière tient à trois champs figés à `None`/`False` dans `_apply_canonical_doc_type_correction`, et elle ne tient que parce que chaque prédicat portant sur un champ propre aux sources exige un signal **positif** (`url_contains`, `self_declared_preprint: True`, `embargo_expired: True`) : les figer à une valeur fausse les éteint. Un prédicat de forme négative sur un de ces champs — la forme existe déjà, `journal_id_present: False` — se vérifierait au contraire sur le canonique, et la règle firerait à tort, en silence. Or `applies_to` est de la donnée déclarative : ses clés énoncent les champs que la règle lit. Filtrer le rejeu canonique sur les règles dont toutes les clés sont répondables à ce niveau rendrait l'exclusion structurelle, et automatique pour toute règle ajoutée ensuite.
