...
- [x] **Test « qui l'appelle » passé sur les endpoints**, dans la phase 1 de [Routes API par ressource](CODE_routes-par-ressource.md) : six chemins sur cent-dix-huit n'ont aucun appelant dans le frontend. Cinq sont des lectures supplantées par des voisines, le sixième est un `DELETE` que l'interface n'offre pas — une fonctionnalité manquante, non un endpoint mort. Le sort de chacun est arbitré dans cette fiche.
