...
- [ ] **Le filtre `hal_status` accepte deux situations où il ne veut rien dire, et rend la liste entière plutôt que de le dire.** Trois de ses quatre valeurs se lisent au regard de la collection HAL d'un laboratoire. Ce filtre n'est offert que par la fiche d'un laboratoire : le demander sans laboratoire, ou pour plusieurs, est une requête contradictoire, qu'un refus devrait accueillir. `_hal_status_clause_sync` rend `None` à la place, et la liste sort non filtrée sous un code 200. Second cas, distinct : un laboratoire unique mais sans collection déclarée — `ok` et `notice` ne produisent alors aucune clause, là où l'absence de collection vaut « hors collection » et devrait se traiter comme telle.
