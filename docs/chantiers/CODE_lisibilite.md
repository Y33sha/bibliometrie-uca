...
- [ ] Motif fonction libre + classe adaptateur qui la délègue. La doc le justifie par la mutualisation entre contextes : le module héberge le SQL en fonctions libres, et plusieurs adaptateurs s'en servent. Là où le module n'a qu'un seul adaptateur, la classe se borne à déléguer et ne mutualise rien : `address_pub_count.py` en est un cas net (`PgAddressPubCountQueries.recompute_pub_count` → `recompute_pub_count`). Recenser les modules à adaptateur unique et replier le SQL dans la classe. Les tests qui appellent la fonction libre passeront par l'adaptateur — un test ne justifie pas l'existence d'une fonction de production, pas plus qu'un CLI oneshot destiné à disparaître.
  Le cas cité en exemple de mutualisation réussie, `subjects.py`, n'en est pas un : `PgSubjectsQueries` délègue à onze fonctions libres, `PgSubjectsAdminQueries` porte son SQL en méthodes, et aucune fonction libre n'est appelée par les deux. Le module illustre deux styles qui cohabitent, non un SQL partagé.
...
- [ ] `subjects.py`, relevé à la relecture de son router. Décisions prises, exécution à cette passe :
  - `count_subjects` nomme deux choses à quatre-vingts lignes d'écart : la fonction libre rend la taille du référentiel (relevée avant et après par la phase pour journaliser les sujets ajoutés), la méthode de `PgSubjectsAdminQueries` rend le total d'une recherche filtrée.
  - `link_publication_subject` (au singulier) et `clear_publication_subjects` (par publication) ne sont exposées par aucun des deux ports et n'ont d'appelants que dans `tests/integration/infrastructure/queries/test_subjects.py` ; le pipeline passe par les variantes en lot. Code mort à supprimer avec les tests qui le maintiennent en vie.
  - La clause de recherche (`usage_count >= :min_count`, plus le `ILIKE` sur `unaccent(label)`) et ses binds sont écrits deux fois, dans `list_subjects` et `count_subjects` : un filtre ajouté d'un seul côté fausse le total en silence. À dédoublonner.
  - Passe docstrings et commentaires : le titre de section `── Co-occurrences ──` coiffe `recompute_usage_counts`, qui compte des publications par sujet ; `purge_orphan_subjects` s'appuie sur l'expression bannie « au fil de l'eau » ; `link_publication_subjects_bulk` documente `rows` comme une liste de tuples quand ce sont des `PublicationSubjectLink` ; `select_publications_to_reingest` plaide son design contre une alternative que personne ne propose et cite une proportion vouée à se périmer ; les retours à la ligne non sémantiques sont partout.
...
- [ ] `SubjectsAdminQueries` annonce un contexte d'administration qu'il ne sert pas : `routers/subjects.py`, public, est son unique consommateur, et aucun router d'admin ne porte les sujets. Le nom juste, `SubjectsQueries`, est déjà pris par le port de la phase d'ingestion (`ports/pipeline/subjects.py`) — et leurs adaptateurs, qui cohabitent dans `infrastructure/queries/subjects.py`, se heurteraient de même. Un des deux côtés doit donc porter un qualificatif : celui de l'API n'en a pas qui ne soit redondant avec son propre dossier, celui du pipeline en a un vrai, sa phase. À trancher avec la scission du module d'adaptateurs notée en 2.6.
- [ ] **`/api/laboratories/*` double `/api/structures/*` pour la même entité** : les lectures « labo » ne filtrent sur aucun type et servent déjà n'importe quelle structure. Fiche dédiée : [Structures et laboratoires](CODE_structures-et-laboratoires.md).
- [ ] **`/api/persons/directory` double `/api/persons`** pour la même entité, et rend les identifiants sous une forme que la page profil, elle, dérive côté client. Fiche dédiée : [Projections de lecture des personnes](CODE_projections-de-lecture-des-personnes.md).
- [ ] **`/api/stats/facets/entity-label` double `/api/publications/facets/entity-label`** : mêmes paramètres, même modèle de réponse, même docstring au mot près, et deux adapters dont le corps d'une ligne appelle la même fonction `entity_label`. Un libellé par identifiant ne dépend d'aucun contexte de filtrage — rien ne distingue les deux routes, déclarées dans deux ports. La facette voisine `facets/entities`, elle, est légitimement dédoublée : ses décomptes dépendent du jeu de filtres. Au passage, le port stats type `kind` en `Literal["publisher", "journal"]`, celui des publications en `str` nu.
