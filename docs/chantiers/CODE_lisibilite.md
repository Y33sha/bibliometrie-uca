...
- [ ] Abréviation `pub_repo` pour `PublicationRepository` : 38 fichiers (normalizers, phases, services, CLI, tests) la portent contre le nom entier `publication_repo`. Rename mécanique tenu par mypy, transverse à toutes les couches — hors de la relecture des routers. Le cas local de `publishers.py`, seul endroit où `pub_repo` désignait un éditeur, est rendu à `publisher_repo` (`6546b8a6`).
- [ ] `sort` est typé `str` dans quatre routers (éditeurs, revues, personnes, publications) là où six valeurs sont connues, `_SORT_MAP.get(...)` avalant l'intrus et le contrat publiant `string`. Famille « vocabulaires fermés » de la fiche [Typage des paramètres de requête](CODE_typage-des-parametres-de-requete.md), que son recensement omet.
