...
- [ ] **`/api/persons/directory` et `/api/persons` listent la même entité**, l'un pour la page publique, l'autre pour l'admin, avec deux modèles de réponse jumeaux (`PersonDirectoryEntry` / `PersonOut`) et le même vocabulaire de filtres. La divergence entre leurs dataclasses avait rendu le filtre département inopérant dès deux valeurs cochées (`6359ea71`) ; la recopie des champs est résorbée, les deux endpoints restent. Même forme que `laboratories`/`structures`.
