...
- [ ] Sweep F : `**kwargs: Any` → signatures explicites sur les adapters `Pg*Queries` (~21 occ. : adapters API `Pg{Persons,Publications,Stats}Queries` + adapters pipeline `Pg*NormalizeQueries`). Éclatement façon `normalize_crossref` (déjà fait).
