...
- [~] Sweep F : `**kwargs: Any` → signatures explicites sur les adapters `Pg*Queries`.
  ...
  - [ ] Adapters pipeline `Pg*NormalizeQueries` (HAL/OpenAlex/Theses/WoS/ScanR, 9 méthodes). Éclatement façon `normalize_crossref` (déjà fait).
