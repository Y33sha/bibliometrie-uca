...
Le plus gros gain : ces deux méthodes sont la racine d'une grande
part de la complexité du pipeline persons.

- Extraire `batch_assign_orphans` vers un use case
  `application/persons.assign_batch_orphan_authorships()` (ou similaire).
  Le repo expose alors des opérations atomiques :
  `link_source_authorships_to_person`, `create_truth_authorship`,
  `set_authorship_person_fk`. Le scoring `SOURCE_PRIORITY` reste
  référencé depuis le SQL si nécessaire mais devient un *paramètre*
  passé par le use case, pas une décision interne.
- Extraire `ensure_truth_authorship` vers
  `application/persons.sync_truth_authorship()` (ou
  `application/authorships_truth.py`). La règle « la meilleure
  position vient de la source de plus haute priorité » devient un
  appel explicite à une fonction de domaine.

Tests : `tests/integration/application/test_persons_*` + tests de
pipeline persons.
...
- Extraire la règle « une authorship vérité est orpheline si aucune
