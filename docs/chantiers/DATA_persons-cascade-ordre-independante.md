...
- **Incarnation nominale : même nom complet, même personne.** Faute d'identifiant ou de validation, le canal nominal retient la règle de base du dénombrement par nom. L'ancre d'identité nominale est une forme normalisée pleine distincte — chaque forme pleine distincte d'un bloc incarne une personne, et deux signatures de forme pleine identique désignent la même. Deux homonymes pleins réellement distincts fondus en une fiche est un faux-merge assumé, réversible seulement par `distinct_persons` ou par l'arrivée d'un identifiant. Une forme réduite se rattache à l'unique ancre pleine qui la subsume, sinon reste orpheline ; un bloc dépourvu de toute forme pleine prend sa forme la plus complète comme ancre. L'ambiguïté ne qualifie donc que les formes réduites face à plusieurs ancres, jamais une forme pleine, qui est une ancre par elle-même.
