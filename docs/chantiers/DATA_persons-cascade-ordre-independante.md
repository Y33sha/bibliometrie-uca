...
- **Must-link au grain signature (`confirmed_authorships`).** Le rattachement manuel d'une signature orpheline à une personne s'inscrit dans une table `confirmed_authorships` (`source_authorship_id` en clé primaire → `person_id`), pendant positif de `rejected_authorships`. Le recalcul l'épingle : la signature garde son `person_id`, immunisée contre la re-dérivation, et la personne visée survit au ramassage tant qu'elle porte une signature épinglée — ce qui couvre la création manuelle, toujours faite en rattachant une orpheline. Le grain signature, non publication, parce que le support recalculé est `source_authorships.person_id` et qu'une signature est un record d'entrée stable là où la publication est dérivée ; l'asymétrie avec le rejet (grain publication) est voulue — exclure vaut pour tout l'article, épingler vise une signature précise. Une confirmation de forme ne suffit pas : au grain (personne, forme), elle ne résout pas *quelle* signature d'une forme ambiguë revient à la personne.
- **Support et grain.** Le support d'appartenance est `source_authorships.person_id`, au grain signature : lui seul porte les affectations divergentes d'une même forme nominale et les détachements. Le grain identité (`author_identifying_keys`) sert le canal identifiant, où la résolution est fonctionnelle. Pas de matérialisation du `person_id` sur `author_identifying_keys` : elle ne serait fonctionnelle que pour les identités identifiant-ancrées — une identité nominale mappe plusieurs personnes homonymes —, dupliquerait le support et pourrait s'en désynchroniser ; le gain de grain vient déjà de la FK `identity_id`. À reconsidérer seulement sur un besoin de lecture mesuré, et alors en matview plutôt qu'en colonne.
- **Recalcul incrémental cloisonné.** Le marquage à recalculer porte sur les records d'entrée (signatures, identités), jamais sur la personne. Le recalcul se limite au composant touché, borné par la clé de blocage nominale (dont la forme reste à trancher, cf. Questions ouvertes). Deux personnes du noyau dans une même composante ne fusionnent pas : c'est un signal d'erreur à remonter, pas à trancher.
...
- [ ] Recalcul limité au composant touché (clé de blocage nominale, cf. Questions ouvertes) : recomposition de `source_authorships.person_id`, incarnation des composantes franches, orphelinage des douteuses, suppression des personnes vidées, préservation du noyau (dont les signatures épinglées `confirmed_authorships`).
...
- **Clé de blocage nominale.** Le patronyme n'est pas un point fixe : une personne peut en changer au cours de sa vie, là où changer de prénom est rare. Un changement complet de patronyme ne partage aucune forme de nom — ces cas ne sont pas rapprochables par le canal nominal et restent des personnes séparées jusqu'à ce qu'un identifiant les relie (fallback conservateur). La question se réduit donc aux signatures qui partagent *déjà* une forme : quelle clé les co-localise sans les éparpiller ni gonfler les blocs ? Le point délicat est le découpage des patronymes composés ou à particule (quel jeton fait « le » nom), qui doit rester cohérent entre la génération des formes et la clé de blocage.
