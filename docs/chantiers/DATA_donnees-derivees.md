...
- [ ] `authorships` en matview — **après** le sidecar (fait) ; ce premier pas (`authorship_structures` matview) la prépare. **Plus gros** : convertir `authorships` regénérerait ses `id`, ce qui forcerait à re-keyer `authorship_structures` — d'où l'ordre inverse. Perf-lecture neutre ; arbitrage = expressibilité de la consolidation (positions/roles/corresponding par priorité) en un SELECT + coût refresh full vs build incrémental daily.
