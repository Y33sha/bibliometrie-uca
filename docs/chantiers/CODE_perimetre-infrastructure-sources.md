...
- **Branches SA / psycopg.** Non retirées à l'aveugle : le chemin psycopg est couvert par un test de régression. Leur simplification est conditionnée au type de connexion réellement utilisé par l'extraction et le cross-import (cf. Questions ouvertes) et se traite avec le déplacement des requêtes en Phase E.
...
- **Type de connexion de l'extraction et du cross-import.** `get_cross_import_dois` et `get_existing_ids` supportent à la fois `Connection` SQLAlchemy et connexion psycopg brute (`row_factory=dict_row`). Le chemin de production connu passe du SQLAlchemy (`run_async`), mais un test de régression couvre psycopg et `get_existing_ids` n'a aucun appelant de production. Trancher le type de connexion cible (SQLAlchemy uniforme, ou psycopg là où l'extraction l'emploie) conditionne la simplification de ces branches et leur forme une fois déplacées en Phase E. `get_existing_ids` sans appelant est peut-être une fonction morte à part entière, à confirmer.
