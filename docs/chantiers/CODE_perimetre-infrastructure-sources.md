...
- **Persistance DB logée dans `common.py`.** `common.py` est surtout de la persistance : UPSERT staging (`upsert_staging`, `upsert_not_found_stub`), pool de DOI cross-import (`get_cross_import_dois`), sélection des rows périmées (`get_stale_rows`, `set_disappeared_by_source_id`), mémorisation des DOI introuvables (`record_doi_not_found`), lecture des identifiants déjà en staging (`get_existing_ids`). Ces requêtes SQL relèvent de `infrastructure/pipeline`, pas du dialogue API.
...
- **Périmètre.** `infrastructure/sources` ne garde que le dialogue avec les API HTTP. La persistance base (staging, cross-import, stale, `doi_lookups`) descend dans `infrastructure/pipeline`. Après extraction de la DB, `common.py` n'a plus de raison d'être et disparaît.
