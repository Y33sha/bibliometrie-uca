...
- [x] Indicateurs sur-mesure : `PhaseMetrics` gagne `details` et `signals`. `extract` remonte `by_source` (trouvés / nouveaux / màj / inchangés / durée par source) ; `resolve_ra` remonte un tableau par Registration Agency (`ra_table` : DOI et préfixes distincts par RA avec %, trié par DOI décroissant, plus les préfixes ajoutés par le run, et une ligne TOTAL ; la part `unknown` inclut les préfixes malformés `doi:`) ; `oa_status` remonte le backlog stale total, les compteurs du run (interrogés / màj / inchangés / non trouvés) et la répartition des publications par statut OA. Le rendu UI gère plusieurs conventions de `details` : `by_source` (table par source), `distributions` (parts triées avec %), `ra_table` (tableau RA), `tables` (avant/après).
- [ ] Indicateurs sur-mesure des autres phases, du plus simple au plus complexe (metadata_correction par type de correction, dédup publications, normalize / cross_imports par source, signaux persons / extract…).
