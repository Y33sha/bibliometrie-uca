...
- [x] Indicateurs sur-mesure : `PhaseMetrics` gagne `details` et `signals`. `extract` remonte `by_source` (trouvés / nouveaux / màj / inchangés / durée par source) ; `resolve_ra` remonte la répartition des DOI candidats par Registration Agency (`distributions` : Crossref / DataCite / unknown, la part `unknown` incluant les préfixes malformés `doi:`). Le rendu UI gère trois conventions de `details` : `by_source` (table), `distributions` (parts avec %), `tables` (avant/après).
- [ ] Indicateurs sur-mesure des autres phases, du plus simple au plus complexe (oa_status par statut, metadata_correction par type, dédup publications, normalize/cross_imports par source…).
