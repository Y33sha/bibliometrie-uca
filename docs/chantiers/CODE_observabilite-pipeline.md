...
- [x] Indicateurs sur-mesure : `PhaseMetrics` gagne `details` et `signals`. `extract` remonte `by_source` (trouvés / nouveaux / màj / inchangés / durée par source) ; `resolve_ra` remonte la répartition des DOI candidats par Registration Agency (`distributions` : Crossref / DataCite / unknown, la part `unknown` incluant les préfixes malformés `doi:`) ; `oa_status` remonte le backlog stale total, les compteurs du run (interrogés / màj / inchangés / non trouvés) et la répartition des publications par statut OA. Le rendu UI gère trois conventions de `details` : `by_source` (table), `distributions` (parts avec %), `tables` (avant/après).
