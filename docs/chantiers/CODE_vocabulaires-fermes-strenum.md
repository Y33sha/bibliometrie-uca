...
- [x] `infrastructure/queries/api/structures.py` : `doc_type = 'article'` (×2) → `DocType.ARTICLE.value`. `relation_type = 'est_tutelle_de'` laissé en chaîne — `structure_relations.relation_type` n'a pas d'enum référençable côté domaine (point de décision, cf. Questions ouvertes).
...
- [x] `infrastructure/queries/pipeline/metadata_correction.py` — doc_types (`'book'` / `'book_chapter'` / `'dataset'`) faits en phase pilote, `source = 'datacite'` → `Source.DATACITE.value` (×2). Reste le seul mapping `relation_type` DataCite → `DoiClusterCase` du `CASE`, suivi à part sous « Exploitation en SQL ».
...
- [ ] `infrastructure/repositories/doi_prefix_repository.py` — les Registration Agencies `'Crossref'` / `'DataCite'` / `'unknown'` en littéraux SQL nus (`get_prefixes_pending_publisher`). Vocabulaire fermé des RA, sans définition référençable ; à arbitrer avec la question de périmètre (vocabulaire applicatif, `doi_prefixes.ra` étant du texte, non un enum PostgreSQL).
...
- **Vocabulaires fermés sans enum référençable.** `structure_relations.relation_type` (`'est_tutelle_de'`…) et les Registration Agencies de `doi_prefixes.ra` (`'Crossref'` / `'DataCite'` / `'unknown'`…) sont des vocabulaires fermés dont les littéraux restent nus faute de définition côté domaine. Leur donner un StrEnum relève du volet définitions ; à arbitrer (la liste des RA n'est peut-être pas fermée, cf. la traîne mEDRA/JaLC…).
