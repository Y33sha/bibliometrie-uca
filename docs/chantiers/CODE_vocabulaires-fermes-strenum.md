...
- [ ] `'labo'` (`structures.structure_type`) codé en dur dans une quinzaine de requêtes — `filters.py` (`no_lab_clause`), `stats/summary.py`, `stats/pivot.py`, `publications/list.py`, `publications/facets.py`, `persons/detail.py`, `persons/admin.py`, plus les CLI `merge_person_duplicates_by_lab.py` et `audit_name_pair_overlaps.py`. L'enum `StructureType` (`domain/structures/structure.py`, `LABO = "labo"`) existe déjà : interpoler `StructureType.LABO.value`. Le réglage d'affichage `laboratories_display_types` n'est **pas** la définition d'un laboratoire (il ne pilote que la page laboratoires) et ne doit pas servir ici. Le `CASE` de tri par type de `structures.py` (`_LIST_ORDER_BY`) était un no-op — les deux consommateurs (page publique labos, page admin) trient par acronyme côté client — et a été retiré plutôt que converti.
