...
- [x] Rapprochement par titre — **erratums** (signal #3 de la phase `relations`) : un erratum sans relation déclarée ni clé partagée est relié (`is_correction_of`) à l'œuvre dont son `title_normalized` reproduit le titre en suffixe (les erratums préfixent « Erratum: », « Corrigendum to »…, donc le titre parent est embarqué verbatim). Garde de longueur > 30, fenêtre d'année [n−2 … n], et garde d'ambiguïté : abstention dès que plus d'un parent **substantiel** (hors `preprint`/`dataset`, simples formes de la même œuvre) porte le même titre. Audit : ~68 erratums orphelins rattachables, précision validée sur échantillon (titre parent embarqué, pas du rapprochement flou).
- [ ] Rapprochement par titre — **preprints** (preprint ↔ publié au même titre sans DOI distinctif, ~84 blocs) et **rétractations** : même mécanisme à étendre (garde auteurs à calibrer pour les preprints, dont le titre n'est pas embarqué après préfixe).
