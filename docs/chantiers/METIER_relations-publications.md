...
- [x] Page détail — **bandeau unique des relations** en pied de header : toutes les relations réunies (plutôt qu'un double système header + sidebar, qui nuirait à la lisibilité) et hiérarchisées par **niveau de gravité** (couleur + ordre) — rétractation = danger (rouge, icône alerte), erratum/correction/avis de préoccupation = warning (ambre, icône alerte), rattachement à une œuvre principale (préprint/supplément/partie/traduction…) = teal, dépendantes + apparentées = gris discret. Bandeau voyant (fond teinté + filet d'accent), titre cliquable de l'œuvre liée. Barème dans `relationTier` (`$lib/labels`). Le bloc central `RelatedPublications.svelte` devient inutilisé — à supprimer une fois le système unique confirmé.
- [x] Pages listes — **familles de doc_types** (`docTypeFamilies`, `$lib/labels` : Publications / Prépublications / Thèses / Données & logiciels / Annexes & divers). Facet « Types » groupée par famille (`FacetDropdown` mode `groups`), et **liste générale `/publications` restreinte par défaut à la famille Publications** (prop `restrictToPublications`) — les pièces dépendantes (erratum, rétractation, peer review) tombent ainsi en « Annexes » donc hors défaut, et s'ajoutent via la facet. Listes embarquées (journal — serveurs de préprints —, éditeur, personne…) laissées permissives. La facet doc_types se calcule en *self-exclusion* côté backend, donc les compteurs des autres familles restent visibles. Famille en facet primaire (option b) reportée ; vue tabulaire totale vs vue liste groupée non traitée.
