...
- [x] **Heuristique `choose_extraction_mode`** : remplacement de `n_orphans < full_fetch_pages` (1 page ≈ 1 requête unitaire) par `n_orphans < 10 × full_fetch_pages` (constante `_ORPHAN_TO_PAGE_RATIO = 10`). Approximation grossière mais nettement plus juste : une page full-fetch tire `per_page` docs avec `label_xml` (TEI verbeux), payload + parsing largement > 1 fetch unitaire. Bascule désormais correctement les umbrellas (PRES_UCA/PRES_CLERMONT) en incremental. Calibrable empiriquement après quelques runs.
