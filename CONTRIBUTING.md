...
- **Hooks git** : deux étages, tous non-mutants (ils constatent et refusent, ne modifient aucun fichier). Le **pre-commit** (garde rapide à chaque commit) lance ruff + ruff format + pytest-unit + uv-lock ; lancer `ruff format .` (et au besoin `ruff check --fix .`) avant `git commit`, sinon le hook refuse le commit. mypy et lint-imports résolvent les imports sur tout le projet (arbre cohérent requis) : ils vivent au pre-push, pas au commit. Le **pre-push** rejoue les vérifications de la CI avant chaque push (deptry, pip-audit, ruff, lint-imports, mypy, build frontend) ; la suite de tests, trop longue, reste à la charge de la CI. Installation : `pre-commit install` puis `pre-commit install --hook-type pre-push`.
