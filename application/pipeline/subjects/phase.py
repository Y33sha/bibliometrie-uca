...
1. **ingestion** (`subjects` + `publication_subjects`) — incrémentale et publication-centrée : ne
   ré-ingère que les publications dont le contenu canonique a changé depuis leur dernière ingestion,
   à partir des `keywords` / `topics` de leurs `source_publications`. Purge en fin les sujets
   devenus orphelins.
2. **co-occurrences** (`subjects.usage_count` + matview `subject_cooccurrences`) — recalcule l'usage
   de chaque sujet et rafraîchit la matview des paires de sujets co-présents sur une même
   publication.

Aucun filtre périmètre : la phase `authorships` a purgé en amont les publications orphelines, donc
`publication_subjects` ne porte que du périmètre et les deux caches en héritent. Idempotente.
"""
