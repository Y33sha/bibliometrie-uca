...
Rattache à son journal une `source_publication` qui porte un DOI mais aucun `journal_id`, lorsque le `doi_prefix` d'un unique journal préfixe ce DOI. Recherche inverse contre la table journals (décision pure `resolve_journal_by_doi`), à cible data-dépendante : hors du DSL des règles unaires => traitement à part.

Idempotent et auto-cicatrisant : on repart du `journal_id` **brut reconstruit**, on n'agit que s'il est NULL, et on re-dérive à chaque run. Si le préfixe DOI ne matche plus (édition du `doi_prefix` du journal) ou n'est plus unique, la colonne est restaurée à NULL et le stash (`raw_metadata.journal_id`) retiré.
"""
...
def _compute_update(
    row: JournalCorrectionRow, journal_prefixes: list[JournalDoiPrefixRow]
...
def compute_updates(
    rows: list[JournalCorrectionRow], journal_prefixes: list[JournalDoiPrefixRow]
...
class JournalByDoiStats:
    """Bilan de la passe : source_publications examinées, et source_publications nouvellement rattachées (journal_id posé)."""
